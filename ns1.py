...
def wlis(children):
	...
	if(msflag==0): 
		...
		if(sc[-1]==server_port):
			...
			time.sleep(5)
		os.system("gnome-terminal -x python ns1.py")
		os._exit(2)
	msflag=0
...
if(signal=="signal"):
	...
	my_client.create("/server/server1",sbind,ephemeral=True,sequence=True) 
	...
	while 1:
		...
		if(k>=ord(server_id[0]) and k<=ord(server_id[1])):
			if(svalue==""):
				...
				connectionSocket.send("RECEVIED")
				no_ch=my_client.get_children("/server/")
				for i in range(0,len(no_ch)):
					if("server2" in no_ch[i]):
						data=my_client.get("/server/"+no_ch[i])
